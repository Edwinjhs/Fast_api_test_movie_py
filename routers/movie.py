...
@movie_router.get('/movies/release_contry/',tags=['MOVIES'],response_model=List[Movie],status_code=200)
def get_movies_by_release_contry(release_contry:str = Query(min_length=3,max_length=15)):
    db = Session()
    result = MovieService(db).get_movies_by_release_contry(id)
    if not result:
        return JSONResponse(status_code=404,content={"message":"No found"})
    return JSONResponse(content=jsonable_encoder(result), status_code=200)
...
def get_movies_by_title(title:str = Query(min_length=3,max_length=15)):
    db = Session()
    result = MovieService(db).get_movies_by_title(id)
    if not result:
        return JSONResponse(status_code=404,content={"message":"No found"})
    return JSONResponse(content=jsonable_encoder(result), status_code=200)
